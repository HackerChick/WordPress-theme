...
function display_upcoming_event( $date, $event, $url, $description){
    echo '<p>' . $date . ': ' . $event . '<br/><a href="' . $url . '" target="_blank"><em>' . $description . '</em></a></p>';
}
...
function display_top_commentors( ){
    echo '<table cellspacing="0" cellpadding="4" border="0" align="right">';
    echo '<tr>';
    display_top_commentor( "The Abbot of Unreason", "commentor-the-abbot-of-unreason.gif", "http://theabbotofunreason.blogspot.com/" );
    display_top_commentor( "Michael Denomy", "commentors-michael.jpg", "http://mdenomy.wordpress.com/" );
    display_top_commentor( "Object-Oriented Sammy", "commentor-object-oriented-sammy.jpg", null );
    display_top_commentor( "Billy Garnet", "commentor-billy.png", "http://billygarnet.wordpress.com/" );
    echo '</tr>';
    echo '<tr>';
    display_top_commentor( "Harold Shinsato", "commentors-harold.jpg", "http://shinsato.com" );
    display_top_commentor( "Tobias Mayer", "commentors-tobias-mayer.png", "http://agilethinking.net/" );
    display_top_commentor( "Itay Maman", "commentors-itay.jpg", "http://javadots.blogspot.com/" );
    display_top_commentor( "joshilewis", "commentors-joshilewis.jpg", "http://twitter.com/joshilewis" );
    echo '</tr>';
    echo '</table>';
}

function display_top_commentor( $name, $image, $url ){
   $closing = "";

   if( $image == null ){
       $image = "commentor-no-avatar.png";
   }
   ...
   echo '<img src="' . get_bloginfo('template_directory') . '/images/pix/' . $image . '" title="' . $name . '" alt="'. $name . '"  class="commentor_avatar">';
   echo $closing . '</td>';
}

/* to reduce cache (default is 12 hours) to 5 minutes (5 * 30 seconds) */
add_filter( 'wp_feed_cache_transient_lifetime', create_function( '$a', 'return 300;' ) );

function display_twitterFeed( $qty){
    include_once(ABSPATH . WPINC . '/feed.php');
    $rss = fetch_feed('http://search.twitter.com/search.atom?q=hackerchick');
    $rss_items = $rss->get_items( 0, $rss->get_item_quantity($qty) );
    if ( !$rss_items ) {
	echo 'Nobody\'s talkin!';
	return;
    }
	...
	$tweet = str_replace( "&apos;", "'", $item->get_content() );

	echo '<p class="tweet">';
        echo '<a href="' .$authorLink . '" target="_blank"><img src="' . $avatar . '" class="twitter_avatar" align="left"></a> ';
	echo $tweet;
	echo '</p>';
    }
    echo '<p class="twitter_tagline"><a href="http://www.twitter.com/HackerChick">&#187; Join the conversation!</a></p>';
}
